function voteup(post_id){
	...
	$('post-' + post_id).down('.karma-voted').update('<img src="/static/images/throbber.gif" />');
	Dajaxice.forums.voteup('Dajax.process', {'post_id': post_id});
	return false;
}

function votecancel(post_id){
	if ($('post-' + post_id).down('.karma_neutral').hasClassName('current') == true) {
		return false;
	}
	$('post-' + post_id).down('.karma-voted').update('<img src="/static/images/throbber.gif" />');
...
function votedown(post_id){
	if ($('post-' + post_id).down('.karma_negative').hasClassName('current') == true) {
		return false;
	}
	$('post-' + post_id).down('.karma-voted').update('<img src="/static/images/throbber.gif" />');
